...
class MongoDB:
    ...
    async def _ensure_indexes(self):
        ...
        try:
            await self.db.links.create_index([('chat_id', 1), ('session_id', 1), ('user_id', 1)])
            await self.db.links.create_index([('chat_id', 1), ('session_id', 1), ('link', 1)])
            await self.db.sessions.create_index([('chat_id', 1), ('is_active', 1)])
            await self.db.safe_list.create_index([('chat_id', 1), ('session_id', 1), ('user_id', 1)])
            await self.db.sr_requests.create_index([('chat_id', 1), ('user_id', 1)])
    ...
    async def create_session(self, chat_id: int, created_by: int) -> Optional[str]:
    ...
    async def reopen_session(self, chat_id: int) -> bool:
        ...
        try:
            # Find the latest closed session for this chat
            latest = await self.db.sessions.find_one(
            ...
            result = await self.db.sessions.update_one(
                {'_id': latest['_id']},
                {'$set': {'is_active': True, 'closed_at': None}}
            )
            return result.modified_count > 0
        except Exception as e:
    ...
    async def clear_session_data(self, chat_id: int, session_id: str) -> bool:
        """Clear all link and SR data for a specific session"""
        try:
            await self.db.links.delete_many({
                'chat_id': chat_id,
                'session_id': session_id
            })
            await self.db.sr_requests.delete_many({
                'chat_id': chat_id,
                'session_id': session_id
            })
            await self.db.safe_list.delete_many({
                'chat_id': chat_id,
                'session_id': session_id
            })
            return True
    ...
    async def enable_ad_tracking(self, chat_id: int, session_id: str) -> bool:
        """Enable ad tracking for a specific session"""
        try:
            result = await self.db.sessions.update_one(
            ...
            if result.modified_count == 0:
                # Fallback: match by active session regardless of session_id field
                result = await self.db.sessions.update_one(
                    {'chat_id': chat_id, 'is_active': True},
                    {'$set': {'ad_tracking_enabled': True}}
                )
    ...
    async def mark_user_links_verified(self, chat_id: int, user_id: int, session_id: str) -> bool:
        """Mark all user's links as verified"""
        try:
    ...
    async def find_user_by_username(self, chat_id: int, session_id: str,
                                    username: str) -> Optional[Dict[str, Any]]:
        """Find a user's record by username (case-insensitive) across collections."""
        username_lower = username.lower()
        try:
            # Search links
            doc = await self.db.links.find_one({
                'chat_id': chat_id,
                'session_id': session_id,
            ...
            # Search safe_list
            doc = await self.db.safe_list.find_one({
                'chat_id': chat_id,
                'session_id': session_id,
                'username_lower': username_lower
            })
            ...
            # Search ad_list (no session scope)
            doc = await self.db.ad_list.find_one({
                'chat_id': chat_id,
                'username_lower': username_lower
            })
            if doc:
            ...
            # Broadest fallback: any chat/session
            doc = (
                await self.db.links.find_one({'username_lower': username_lower}) or
                await self.db.safe_list.find_one({'username_lower': username_lower}) or
                await self.db.ad_list.find_one({'username_lower': username_lower})
            )
            return doc
        except Exception as e:
